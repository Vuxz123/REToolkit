...
Assert-Contains $re 'AssetRipper      = Join-Path $Tools "AssetRipper\AssetRipper.exe"' "re.ps1 should know the AssetRipper executable path."
Assert-Contains $re '"assetripper"' "re.ps1 should expose an AssetRipper launcher command."
Assert-Contains $re 'Start-DetachedGuiProcess -FilePath $ToolPaths.AssetRipper' "re.ps1 should launch AssetRipper as a visible detached GUI process."
...
Assert-Contains $processModule 'function Start-DetachedGuiProcess' "process module should expose a visible detached GUI launcher."
...
Assert-Contains $installer '$InstallDotNetRuntime = $true' "-All should include .NET runtime installation for Il2CppDumper."
...
Assert-Contains $installer '[int]$DotNetRuntimeMajor = 8' "installer should default Il2CppDumper to the supported net8.0 package."
Assert-Contains $installer 'function Install-DotNetRuntime' "installer should be able to install the .NET runtime required by Il2CppDumper."
Assert-Contains $installer 'Microsoft.DotNet.Runtime.$Major' "installer should install the core .NET runtime, not an unrelated desktop runtime."
Assert-Contains $installer 'winget install --id $packageId -e --accept-source-agreements --accept-package-agreements' "installer should install .NET Runtime with winget when missing."
Assert-Contains $installer 'Microsoft\.NETCore\.App\s+8\.' "installer should detect Microsoft.NETCore.App 8.x for Il2CppDumper net8.0."
Assert-Contains $installer 'Run with -InstallDotNetRuntime' "installer should guide users to the dedicated .NET runtime install flag."
...
Assert-Contains $installer '^ghidra_mcp_bridge-.+\.whl$' "installer should find the release bridge wheel."
...
Assert-Contains $installer 'function Install-GhidraMcpBridgeWheel' "installer should install the MCP bridge wheel into a local venv."
Assert-NotContains $installer 'bridge_mcp_ghidra.py' "installer should not depend on the retired standalone bridge_mcp_ghidra.py asset."
Assert-NotContains $installer 'requirements.txt' "installer should not depend on the retired standalone requirements.txt asset."
...
Assert-NotContains $installer 'WindowsDesktop runtime (needed by Il2CppDumper)' "installer should not claim Il2CppDumper requires WindowsDesktop runtime."
...
Assert-Contains $pipelineModule 'has no files newer than the export start. Check the log file for details' "Export-AssetRipperUnityProject should verify a fresh export actually wrote files newer than the export start, since GameFileLoader can silently no-op on a re-run without emptying the output dir."
Assert-Contains $pipelineModule '-TimeoutSec 0' "AssetRipper Load/Export POSTs should use an unbounded HTTP timeout since export time scales with build size."
Assert-Contains $pipelineModule 'CreateSubfolder = "false"' "Export-AssetRipperUnityProject should send CreateSubfolder as the literal string `"false`", not a PowerShell boolean, since AssetRipper compares it as an exact string."
Assert-Contains $pipelineModule 'Get-Process -Id $ProcessId -ErrorAction SilentlyContinue' "Wait-AssetRipperHttpReady should detect the AssetRipper process crashing before its web server becomes ready."
...
Assert-Contains $uiModule 'assetripper [args...]' "UI module should document the AssetRipper GUI launcher."
...
Assert-Contains $readme '-InstallDotNetRuntime' "README should document .NET runtime installation for Il2CppDumper."
...
Assert-Contains $readme 'assetripper [args...]' "README should document AssetRipper GUI launcher."
...
Assert-Contains $tutorial '-InstallDotNetRuntime' "Tutorial should document .NET runtime installation for Il2CppDumper."
...
Assert-Contains $promptInstallGhidraSkill '## Install REToolkit Skills' "Prompt should include a dedicated REToolkit skill install section."
Assert-Contains $promptInstallGhidraSkill 'These REToolkit skills are plain Markdown instruction folders.' "Prompt should explain REToolkit skills are agent-neutral instruction folders."
Assert-Contains $promptInstallGhidraSkill 'For non-Codex agents' "Prompt should explain how other agents can load REToolkit skills."
Assert-Contains $promptInstallGhidraSkill '$CODEX_HOME' "Prompt should explain where Codex skills are installed."
Assert-Contains $promptInstallGhidraSkill 'Copy-Item -LiteralPath $src -Destination $dst -Recurse' "Prompt should provide a concrete local skill copy command."
Assert-Contains $promptInstallGhidraSkill 'Restart Codex to pick up newly installed skills.' "Prompt should tell users to restart Codex after skill installation."
Assert-Contains $promptInstallGhidraSkill 'Il2CppDumper cannot find .NET' "Prompt should troubleshoot missing .NET runtime for Il2CppDumper."
...
Assert-Contains $promptInstallGhidraSkill '.\re.ps1 assetripper' "Prompt should document AssetRipper GUI launch."
